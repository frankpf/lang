...
export class Parser {
	...
	private finishLetDeclarationStatement(): Ast.Stmt {
		...
		let typeIdentifier: Ast.ToInfer | Token = Ast.ToInfer
		if (this.match(TokenType.Colon)) {
			typeIdentifier = this.consume(TokenType.Identifier, 'Expected type name after :')
		}
		...
		return new Ast.Stmt.LetDeclaration(identifier, typeIdentifier, initializer)
	}
	...
	*/
	...
	private primary(): Ast.Expr {
		...
		if (this.match(TokenType.False)) {
			return new Ast.Expr.Literal(false, this.previous(), TcType.Boolean)
		}

		if (this.match(TokenType.True)) {
			return new Ast.Expr.Literal(true, this.previous(), TcType.Boolean)
		}

		if (this.match(TokenType.Nil)) {
			return new Ast.Expr.Literal(null, this.previous(), TcType.Nil)
		}

		if (this.match(TokenType.IntegerLit)) {
			return new Ast.Expr.Literal(this.previous().literal, this.previous(), TcType.Integer)
		}

		if (this.match(TokenType.DoubleLit)) {
			return new Ast.Expr.Literal(this.previous().literal, this.previous(), TcType.Double)
		}

		if (this.match(TokenType.StringLit)) {
			return new Ast.Expr.Literal(this.previous().literal, this.previous(), TcType.String)
		}
	...
	private error(token: Token, message: string): ParseError {
		error(token, message)
		console.log(token)
		this.ctx.sourceErrorReporter.report('parse error', message, token)
