...
export class Scanner {
	...
	private start = 0 // Used to determine token start

	private lineStart = 0 // Used to calcualte sourcecodeLines, see increaseLines()
	...
	private scanTokens(): Token[] {
		...
		// FIXME: Refactor this, there's duplication with handleNewline
		const lastToken = this.tokens[this.tokens.length - 1]
		if (Scanner.lineEndingTokenTypes.has(lastToken.type)) {
			this.tokens.push(new Token(TokenType.Semicolon, ';', null, this.line, this.column))
		}
	...
	private handleNewline() {
		// Handle empty/comment lines in beginning of file
	...
	private increaseLine() {
		const text = this.source.substring(this.lineStart, this.current)
		this.sourcecodeLines.push(text)
